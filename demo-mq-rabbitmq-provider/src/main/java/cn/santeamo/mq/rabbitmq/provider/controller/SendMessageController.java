...
public class SendMessageController {
    ...
    public String sendDirectMessage() {
        ...
        rabbitTemplate.convertAndSend(
            ...
            getMessageMap("sendDirectMessage").toString());
        return "ok";
    }

    @GetMapping("/sendTopicOneMessage")
    public String sendTopicOneMessage() {
        // 将消息携带绑定键值：topic.one 发送到交换机 topicExchange
        rabbitTemplate.convertAndSend(
            RabbitConst.Topic.EXCHANGE,
            RabbitConst.Topic.One.ROUTING_KEY,
            getMessageMap("sendTopicOneMessage").toString());
        return "ok";
    }

    @GetMapping("/sendTopicTwoMessage")
    public String sendTopicTwoMessage() {
        // 将消息携带绑定键值：topic.two 发送到交换机 topicExchange
        rabbitTemplate.convertAndSend(
            RabbitConst.Topic.EXCHANGE,
            RabbitConst.Topic.Two.ROUTING_KEY,
            getMessageMap("sendTopicTwoMessage").toString());
        return "ok";
    }
    ...
    public String sendFanoutMessage() {
        ...
        rabbitTemplate.convertAndSend(
            RabbitConst.Fanout.EXCHANGE,
            null,
            getMessageMap("sendFanoutMessage").toString());
        return "ok";
    }
